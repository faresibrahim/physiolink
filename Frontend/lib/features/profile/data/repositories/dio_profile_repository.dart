import 'package:dartz/dartz.dart';
import 'package:dio/dio.dart';
import 'package:practice/core/error/app_failure.dart';
import 'package:practice/features/profile/domain/entities/patient.dart';
import 'package:practice/features/profile/domain/repositories/profile_repository.dart';

class DioProfileRepository implements ProfileRepository {
  final Dio _dio;
  DioProfileRepository(this._dio);

  @override
  Future<Either<AppFailure, Patient>> getProfile(String patientId) async {
    try {
      final response = await _dio.get('/api/v1/patients/$patientId/profile');
      return Right(Patient.fromJson(response.data));
    } catch (ex) {
      return Left(_mapError(ex));
    }
  }

  @override
  Future<Either<AppFailure, void>> updateProfile({
    required String patientId,
    required String firstName,
    required String lastName,
    required String phoneNumber,
  }) async {
    try {
      await _dio.put(
        '/api/v1/patients/$patientId/profile',
        data: {
          'firstName': firstName,
          'lastName': lastName,
          'phoneNumber': phoneNumber,
        },
      );
      return const Right(null);
    } catch (ex) {
      return Left(_mapError(ex));
    }
  }

  AppFailure _mapError(Object ex) {
    if (ex is DioException) {
      if (ex.type == DioExceptionType.connectionError) {
        return const NetworkFailure();
      }
      if (ex.response?.statusCode == 401) return const AuthFailure();
      return ServerFailure(ex.response?.statusCode ?? 500);
    }
    return const ServerFailure(500);
  }
}
